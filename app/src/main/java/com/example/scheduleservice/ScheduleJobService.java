    ...
    public boolean onStartJob(JobParameters params) {
        new Thread(new Runnable() {
            ...
            if(isAlive){
                ...
                int count = (dbItemDataList.size() >= scheduleItemDataList.size()) ? scheduleItemDataList.size():dbItemDataList.size(); //проверка чтобы не вылезти за границы массивов
                for (int i=0;i<count;i++){
                    if(!(scheduleItemDataList.get(i).getFileName().trim().equals(dbItemDataList.get(i).getFileName().trim()))){
